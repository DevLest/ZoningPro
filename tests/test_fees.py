...
def test_residential_200k_plus_matches_workbook_sample():
    """Excel Residential-200K+: K17=excess, K18=K17*1%, K19=K18/10, LC=720+K19; P17 same as C18."""
    cost = 2_045_831.0
    r = compute_fees("residential_200k_plus", cost)
    excess = cost - 200_000.0
    k18 = excess * 0.01
    k19 = k18 * 0.1
    expected_lc = 720.0 + k19
    ...
    assert r.zoning_cert == 720.0
    assert r.total == pytest.approx(expected_lc + 720.0, abs=0.02)
...
def test_institutional_2m_plus_zoning():
    lot = 52146.0
    r = compute_fees("institutional_2m_plus", 3_000_000.0, lot_area_sqm=lot)
    assert r.zoning_cert == pytest.approx((lot / 10_000.0) * 720.0)


def test_special_use_2m_plus():
    cost = 3_058_388.05
    r = compute_fees("special_use_2m_plus", cost)
